      ...
      <div className="m-auto">
        {!loading && !loadingAfter ? (
          <div className="mt-20 grid grid-cols-3 gap-y-24 w-auto h-[750px] overflow-scroll no-scrollbar">
            {books.map((book,key)=>{
              return (
                <Link href={`/bookDetails/${book.id}`} onClick={()=>setLoadingAfter(true)} key={key} className="flex flex-col items-center justify-center relative mx-20 hover:scale-110 transition-all">
